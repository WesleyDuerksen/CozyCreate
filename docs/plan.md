...
What it does: ensures mods are present in `server/content/mods/` (downloading missing Modrinth jars from `mods/*.pw.toml` URLs), starts the stack via `docker compose up -d`, polls the container healthcheck until healthy or timeout, parses `server/data/logs/latest.log` for ERROR/FATAL against `KNOWN_OK_PATTERNS`, then `docker compose down`.

Layout: `server/content/` is the authored pack (mods, config, kubejs, patchouli_books) and is git-tracked. `server/data/` is instance state (world, logs, libraries, player files) generated by the container and gitignored entirely.

**Pass criteria:** container reports `healthy` + zero unfiltered errors/fatals in `server/data/logs/latest.log`.
...
**0.3.1**
Memory optimization pass — ModernFix perf/bugfix mixins enabled in `server/content/config/modernfix-mixins.properties`:
