...
- [Commit workflow — validate first, capture configs, check ignores](feedback_commit_workflow.md) — Required steps before every commit: validate.sh, git status server/data/config, update .gitignore and .packwizignore for any new dirs, verify index.toml
...
- [Config source of truth is server/data/config/](project_config_source_of_truth.md) — Root config/ is gitignored and regenerated by export.sh; all mod config goes in server/data/config/; Patchouli books are an exception (see below)
